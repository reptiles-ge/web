...
function compactAsset(
  ...
  return { path: assetPath, width: entry.width, height: entry.height, widths };
}
...
async function generateDataFile(
  ...
  const assets = Object.entries(manifest.entries)
    ...
    .map(([key, entry]) => [
      ...
      compactAsset(key, entry, storage, prefix),
    ]);
...
export const optimizedFormats = ${JSON.stringify(FORMATS)} as const;
