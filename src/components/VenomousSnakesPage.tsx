            ...
            <div className="mt-12 grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
              {species.map((item, index) => (
                <Reveal key={item.id} delay={index * 60}>
                  <SpeciesGuideCard species={item} locale={locale} />
                </Reveal>
              ))}
            </div>
          </div>
...
function SpeciesGuideCard({
  ...
  const regions = getRegionsForSpecies(species.id)
    ...
    .slice(0, 2);
  const extra = Math.max(0, getRegionsForSpecies(species.id).length - regions.length);
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-x-0 bottom-0 p-5 sm:p-6">
        ...
        <p className="mt-4 text-[12px] italic text-white/50">
          {species.scientificName}
        </p>
        <h3 className="mt-1 font-display text-[22px] font-semibold leading-tight text-white sm:text-[24px]">
          {species.commonName}
        </h3>
        ...
        <span className="mt-4 inline-flex items-center gap-1.5 text-[13px] font-medium text-white/80">
          {t("openProfile")}
