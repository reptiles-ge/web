...
async function generateDataFile(
  ...
  const assets = Object.entries(manifest.entries)
    ...
    .map(([key, entry]) => [
      byKey.get(key) as string,
      {
        width: entry.width,
        height: entry.height,
        sources: [...entry.derivatives]
          .sort((a, b) =>
            a.format === b.format ? a.width - b.width : a.format.localeCompare(b.format),
          )
          .map((derivative) => ({
...
export const optimizedImages: Record<string, OptimizedImageAsset> = ${JSON.stringify(
